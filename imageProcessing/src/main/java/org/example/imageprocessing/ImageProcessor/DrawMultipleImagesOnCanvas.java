...
public class DrawMultipleImagesOnCanvas {
    ...
    private Queue<ImageData> imageQueue = new LinkedBlockingQueue<>();
...
//    private Canvas canvas;
    ...
    public void initialize(Stage primaryStage) {
        ...
        new AnimationTimer() {
            ...
            public void handle(long now) {
                ...
                if (!isDrawing && !imageQueue.isEmpty()) {
                    new Thread(() -> {
                        try {
                            drawNextImage();
                        } finally {
    ...
    private void drawNextImage(){
        ...
        ImageData imageData = imageQueue.poll();

        Platform.runLater(() -> {
            if(imageData != null){
                this.gc.drawImage(SwingFXUtils.toFXImage(imageData.getImage(), null),
                        imageData.getI(), imageData.getJ(), imageData.getWidth(), imageData.getHeight());
